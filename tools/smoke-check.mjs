...
function checkAssetSizeReport() {
  ...
  assert(
    ...
    'asset-size-report.mjs: catalog-return-cache.js must stay lazy-loaded outside startup profiles'
  );
  assert(
...
async function checkStaticGuards() {
  ...
  assert(
    movieEditorJs.includes('async function insertMovieRecord(') &&
      ...
      !appJs.includes('markLocalDataMutation(`movie-create:${insertedMovie.id}`)') &&
      !appJs.includes('markLocalDataMutation(`movie-update:${editingMovieId}`)') &&
      !appJs.includes('reloadMoviePageData(editingMovieId)'),
  ...
  assert(
    movieEditorJs.includes('function resetMovieFormToCreateMode(') &&
      ...
      movieEditorJs.includes('function setMoviePosterFileUiState(') &&
      ...
      appJs.includes('movieEditorController?.setMoviePosterFileUiState(moviePosterImagesDraft)') &&
      !appJs.includes('movieForm.reset();') &&
      !appJs.includes('posterFileName.textContent') &&
  ...
  assert(
    appJs.includes("import(getLazyFeatureModuleUrl('profile-settings-actions.js'))") &&
      !appJs.includes('function extractAvatarStoragePath(') &&
      !appJs.includes('function uploadAvatarBlob(') &&
      !appJs.includes("select('id')\n    .eq('display_name_normalized'") &&
      profileSettingsActionsJs.includes('function isDisplayNameAvailable(') &&
  ...
  assert(
    appJs.includes("import(getLazyFeatureModuleUrl('catalog-return-cache.js'))") &&
      !appJs.includes('function getCatalogSessionSnapshotSignature(') &&
      !appJs.includes('function readCatalogSnapshot(') &&
      catalogReturnCacheJs.includes('function getCatalogSessionSnapshotSignature(') &&
      catalogReturnCacheJs.includes('function readCatalogSnapshot(') &&
      catalogReturnCacheJs.includes('function writeCatalogSnapshot('),
  ...
  assert(
    appJs.includes("import(getLazyFeatureModuleUrl('catalog-url-state.js'))") &&
      ...
      catalogUrlStateJs.includes('function getCatalogUrlValueByAlias(') &&
      catalogUrlStateJs.includes('function getCatalogUrlSearchParamsFromState('),
    'app.js: catalog URL state mechanics should stay in catalog-url-state.js'
  );
  assert(
