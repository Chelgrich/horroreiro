...
6. CSS: in progress. First passes split catalog-only shell/control styles and catalog page-level CSS variables into `catalog-page.css` while keeping shared movie card primitives in `styles.css`; continue tightening remaining global card/form/shared values.
