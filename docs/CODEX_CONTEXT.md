...
6. CSS: in progress. First passes split catalog-only shell/control styles and catalog page-level CSS variables into `catalog-page.css`, then lazy-split movie add/edit modal-only styles into `movie-editor.css`. Shared movie card primitives stay in `styles.css`; continue tightening remaining global card/form/shared values.
