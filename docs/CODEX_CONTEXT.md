...
3. Supabase payload/select audit: in progress. Notifications split lightweight movie links from digest movie cards; profile rails use compact activity row selects; person/director pages use explicit `PEOPLE_*_SELECT` profiles. Continue with catalog and movie detail RPC fallback so each page fetches only fields it actually needs.
