...
3. Supabase payload/select audit: in progress. Notifications split lightweight movie links from digest movie cards; profile rails use compact activity row selects; person/director pages use explicit `PEOPLE_*_SELECT` profiles; poster gallery display reads use a compact `MOVIE_POSTER_IMAGE_DISPLAY_SELECT`. Continue with catalog payload and any remaining movie detail fallback edges so each page fetches only fields it actually needs.
