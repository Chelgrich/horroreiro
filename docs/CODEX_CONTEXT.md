...
`user-page.css` owns profile page-only layout styles loaded by `boot-loader.js` only for `data-app-page="user"`: profile hero, display-name/settings button, follow button, profile stats/rank medals, taste cards, profile sections, and the admin password panel. Shared profile avatar primitives, shared back links, and shared movie rail/card primitives stay in `styles.css` because profile settings, secondary pages, notifications, and movie detail rails reuse them.
