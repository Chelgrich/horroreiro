...
`movie-editor.js` is lazy-loaded by the shared movie add/edit modal and owns form draft reading, validation, insert payload building, movie row insert/update writes, update changed-field/relation diffing, poster draft order helpers, poster upload/save result preparation, and create/update save-plan decisions. `app.js` keeps the modal DOM bridge, poster/manual-similar editor UI state/rendering, relation/manual-similar/gallery writes, and post-save page/cache synchronization.
...
2. Movie editor: in progress; continue moving modal submit orchestration, relation/manual-similar/gallery write sequencing, and post-save synchronization out of `app.js` after the current helper/relation/poster-save-plan/movie-row-write split.
