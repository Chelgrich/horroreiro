...
async function loadDirectorPageController() {
  ...
  if (!directorPageControllerPromise) {
    directorPageControllerPromise = Promise.all([
      import(getLazyFeatureModuleUrl('director-page.js')),
...
function getCatalogCardControllerContext() {
  return {
    document,
    getDefaultSearchQuery: () => searchInput?.value || '',
    getSearchQueryWords,
    createSearchHighlighter,
    escapeHtml,
    getHasCurrentUser: () => Boolean(currentUser),
    getIsAdmin: () => Boolean(isAdmin),
    getCurrentUserRating,
    getCurrentUserMovieState,
    getCatalogMovieMeta,
    getMatchedSearchAlias,
    getMovieAverageRating,
    getMovieVotesCount,
    getVotesLabel,
    getCatalogProfileRatingHtml,
    getMoviePreferredPosterUrl,
    isPosterLoaded: posterUrl => Boolean(posterUrl && loadedPosterUrls.has(posterUrl)),
    getPosterImageAttributeHtml,
    bindPosterLoadState,
    isRatingRequestInFlight: movieId => ratingRequestInFlight.has(String(movieId)),
    isWatchlistRequestInFlight: movieId => watchlistRequestInFlight.has(String(movieId))
  };
}

function loadCatalogCardController() {
  if (!catalogCardControllerPromise) {
    catalogCardControllerPromise = import(getLazyFeatureModuleUrl('catalog-cards.js'))
      .then(module => {
        catalogCardController = module.createCatalogCardController(getCatalogCardControllerContext());
        return catalogCardController;
      })
      .catch(error => {
        catalogCardControllerPromise = null;
        catalogCardController = null;
        throw error;
      });
  }

  return catalogCardControllerPromise;
}

function getCatalogCardController() {
  if (!catalogCardController) {
    throw new Error('Catalog card module is not loaded.');
  }
...
function createMovieCardRenderContext(searchQuery = searchInput?.value || '') {
  return getCatalogCardController().createMovieCardRenderContext(searchQuery);
}
...
function createMovieCard(
  ...
  return getCatalogCardController().createMovieCard(movie, renderContext, renderOptions);
}
